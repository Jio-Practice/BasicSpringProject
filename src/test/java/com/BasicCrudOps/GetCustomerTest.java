...
import static com.Codes.ErrorEnums.EMAIL_NOT_FOUND_INVALID_CODE;
import static com.Codes.ErrorEnums.MOBILE_NOT_FOUND_INVALID_CODE;
...
public class GetCustomerTest {
	...
	public void init__() {
		...
		Mockito.when(customerRepository.findByEmailId(TestHelper.INVALID_MAIL))
				.thenThrow(new UserNotFoundException(EMAIL_NOT_FOUND_INVALID_CODE));
		Mockito.when(customerRepository.findByMobileNo(TestHelper.INVALID_MOBILE))
				.thenThrow(new UserNotFoundException(MOBILE_NOT_FOUND_INVALID_CODE));
	}
	...
	public void testForAlreadySavedValidUser() throws Exception {
		...
		assert (isEqual(result, MOBILE_NOT_FOUND_INVALID_CODE));
		assert (isStatusBadRequest(result));
		result = getMvcResult(4);
		assert isEqual(result, EMAIL_NOT_FOUND_INVALID_CODE);
		assert (isStatusBadRequest(result));
	}
	...
	private boolean isEqual(MvcResult actual, ErrorEnums expected) throws UnsupportedEncodingException {
		return actual.getResponse().getContentAsString().equals(expected.toString());
	}

	private boolean isEqual(MvcResult actual, Customer expected) throws UnsupportedEncodingException {
		return actual.getResponse().getContentAsString().equals(expected.toString());
	}

	private void callCreate() throws Exception {
		mvc.perform(MockMvcRequestBuilders.post("/create").content(TestHelper.getAnyValidCustomer().toString())
