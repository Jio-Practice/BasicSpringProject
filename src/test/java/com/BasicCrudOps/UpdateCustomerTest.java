...
import static com.Codes.ErrorEnums.SUCCESS_CODE;
...
public class UpdateCustomerTest {
	...
	public void init__() {
		...
		Mockito.doAnswer((InvocationOnMock invocation) -> {
			if (!saved)
				...
				validCustomer.setAddress(TestHelper.NEW_ADDRESS);
			return null;
	...
	public void testForCorrectUpdateForNonExistingUSer() throws Exception {
		...
		assert (isEquals(result, MOBILE_NOT_FOUND_INVALID_CODE));
		assert (validCustomer.getAddress().equals(TestHelper.VALID_ADDRESS));
	}

	private boolean isEquals(MvcResult result, ErrorEnums expected) throws UnsupportedEncodingException {
		return result.getResponse().getContentAsString().equals(expected.toString());
	}
